...
import jp.osdn.gokigen.aira01b.getimage.JpegDownloaderFromOPC;
import jp.osdn.gokigen.aira01b.getimage.LargeFileDownloaderFromOPC;
		...
		try
		{
			...
			if ((activity != null) && (camera != null))
			{
				if (suffix.endsWith(JPEG_SUFFIX))
				{
					...
					imageDownloader.startDownload(contentInfoEx, downloadSize, isShare);
				}
				else
				{
					// 動画・RAWファイルの取得
					LargeFileDownloaderFromOPC fileDownloader = new LargeFileDownloaderFromOPC(activity, camera);
					fileDownloader.startDownload(contentInfoEx, isShare);
				}
			}
			else
			{
