...
public class PreferenceFragment extends PreferenceFragmentCompat implements SharedPreferences.OnSharedPreferenceChangeListener, PreferenceSynchronizer.IPropertySynchronizeCallback, IPreferenceIntentCaller
{
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        ...
        if (key.equals(ICameraPropertyAccessor.RAW)) {
            ...
            return;
        } else if (key.equals(ICameraPropertyAccessor.SHOOT_ONLY_CAMERA)) {
            boolean value = preferences.getBoolean(key, true);
            setBooleanPreference(key, key, value);
            return;
        } else if (key.equals(ICameraPropertyAccessor.SHOW_SAMPLE_IMAGE)) {
            setShowSampleImageModePreference();
            return;
        }

        String value = preferences.getString(key, "");
        setListPreference(key, key, value);
        String propertyKey = convertKeyFromPreferenceToCameraPropertyKey(key);
        if (propertyKey != null) {
            setCameraProperty(propertyKey, value);
        }
    }
        ...
        if (requestCode == ICameraPropertyAccessor.SELECT_SAMPLE_IMAGE_CODE)
        {
            switch (resultCode)
            {
                case Activity.RESULT_OK:
                    ...
                    findPreference("select_sample_image").setSummary(fileName);
                    SharedPreferences.Editor editor = preferences.edit();
                    editor.putString(ICameraPropertyAccessor.SELECT_SAMPLE_IMAGE, filePath);
                    editor.apply();
                    Log.v(TAG, "onActivityResult() image File : " + fileName + " (" + filePath + ")");
    ...
    @Override
    public void selectSampleImageFromGallery()
    {
        Intent intent = new Intent(Intent.ACTION_PICK);
        intent.setType("image/*");
        startActivityForResult(intent, ICameraPropertyAccessor.SELECT_SAMPLE_IMAGE_CODE);
    }
}
