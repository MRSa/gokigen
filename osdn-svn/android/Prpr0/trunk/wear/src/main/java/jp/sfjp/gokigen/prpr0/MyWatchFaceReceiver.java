...
class MyWatchFaceReceiver
{
     ...
     */
    static final String PATH_WITH_FEATURE = "/gokigen/prpr/feature";
    private static final String PATH_WITH_IMAGE = "/gokigen/prpr/image";
    private static final String KEY_COMMAND_FILE_CLEAR = "CMD_FILE_CLEAR";
    private static final String KEY_COMMAND_CLEAR_RESULT = "CMD_CLEAR_RESULT";
    private static final int COMMAND_VALUE_ALL = -1;
    ...
    MyWatchFaceReceiver(GoogleApiClient client)
    {
        mGoogleApiClient = client;
        fileUtility = new ExtStorageFileUtil("/prpr");
    }
        ...
        try
        {
            for (DataEvent dataEvent : dataEvents)
            {
                ...
                if (dataItem.getUri().getPath().equals(PATH_WITH_IMAGE))
                {
                    ...
                    String fileName = outputBitmap(bitmap);
    ...
    void parseDataMap(DataMap config)
    {
        if (Log.isLoggable(TAG, Log.DEBUG))
        {
     ...
     */
    private String outputBitmap(Bitmap bitmap)
    {
        String fileName = "";
        if (bitmap == null)
        {
        ...
        fileName = fileUtility.putPngImageFromBitmap(directory, bitmap);
        if (Log.isLoggable(TAG, Log.DEBUG))
        {
