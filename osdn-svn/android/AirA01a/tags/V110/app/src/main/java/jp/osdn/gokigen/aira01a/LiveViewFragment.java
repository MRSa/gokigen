...
*/
    ...
    public void detectedHumanFaces()
    {
        ...
        for(Map.Entry<String, RectF> face : faceMap.entrySet())
        {
            ...
            // タッチシャッターが有効な場合には、顔に合わせてシャッターを切る
            if (cameraController.getTouchShutterStatus())
            {
                cameraTakePicture.takePictureWithPoint(new PointF(face.getValue().centerX(), face.getValue().centerY()));
                break;
            }
            ...
            {
                cameraTakePicture.lockAutoFocus(new PointF(face.getValue().centerX(), face.getValue().centerY()));
            }
            else
            {
            }
