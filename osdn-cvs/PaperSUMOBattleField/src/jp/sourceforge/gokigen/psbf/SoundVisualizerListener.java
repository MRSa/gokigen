...
//    private Activity parent = null;  // 親分
	...
	private IDataCaptureListener dataListener = null;
     ...
     * @param listener
     */
    public void setDataCaptureListener(IDataCaptureListener listener)
    {
    ...
    public void drawOnCanvas(Canvas canvas)
    {
    	...
    	{
    		// 例外発生...でもそのときには何もしない
    		Log.v(PSBFMain.APP_IDENTIFIER, "drawOnCanvas() ex: " + ex.toString());
    	}
    ...
    public void drawFft(Canvas canvas)
    {
        ...
        {
        	// サウンド解析情報を通知する
        	dataListener.onMusicDataReceived(max, maxValue);
        }
    }
...
//        height = (canvas.getHeight() / (float) 256 * 2 / 3);// * (-1);
    ...
    {
    	boolean ret = false;
        Log.v(PSBFMain.APP_IDENTIFIER, "SoundVisualizerListener::onTouchEvent()");
        if (dataListener != null)
        {
        	// 画面タッチ情報を通知する
        	ret = dataListener.onTouchEventReceived(event);
        }
    	return (ret);
    }
    ...
    {
        //Log.v(PSBFMain.APP_IDENTIFIER, "SoundVisualizerListener::onFftDataCapture() : " + fft.length);	
        fftBuffer = fft;
    ...
    {
        //Log.v(PSBFMain.APP_IDENTIFIER, "SoundVisualizerListener::onWaveFormDataCapture() : " + waveform.length);
        waveBuffer = waveform;
        this.samplingRate = samplingRate;
    ...
    {
    	/** タッチイベントを受信した時の処理 **/
        public abstract boolean onTouchEventReceived(MotionEvent event);

        /** 音楽データを受信した時の処理 **/
        public abstract void onMusicDataReceived(float current, float max);
    }    
}
