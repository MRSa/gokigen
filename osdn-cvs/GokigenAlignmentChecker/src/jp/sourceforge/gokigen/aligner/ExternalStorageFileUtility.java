...
public class ExternalStorageFileUtility 
{
    ...
    public ExternalStorageFileUtility(String offsetDir)
    {
        ...
        boolean ret = prepareBaseDirectory(offsetDir);
        if (ret == false)
        {
        	Log.v(Main.APP_IDENTIFIER, "prepareBaseDirectory() : false (" + baseDirectory + ")");
        }
    }
    ...
    private boolean prepareBaseDirectory(String offsetDir)
    {
        ...
        {
            Log.v(Main.APP_IDENTIFIER, "prepareBaseDirectory() : " + ex.getMessage() + " " + ex.toString());
        }
        baseDirectory = Environment.getExternalStorageDirectory().getPath();
        return (false);
    }
    ...
    public FileOutputStream openFileStream(String fileName, boolean isAppend)
    {
        try
        {
            ...
            Log.v(Main.APP_IDENTIFIER, "openFileStream() : " + targetName);
            FileOutputStream fileStream = new FileOutputStream(targetName, isAppend);
            return (fileStream);
        }
        catch (Exception e)
        {
            Log.v(Main.APP_IDENTIFIER, "ERR>openFileStream() : " + e.getMessage() + " " + e.toString());
        }
        return (null);
    }    
